...
const Navbar = ({ onSelectCategory, onSearch }) => {
  ...
  const fetchData = async (value) => {
    try {
      const response = await axios.get("https://ff-update-backend-6.onrender.com/api/products");
  ...
  const handleChange = async (value) => {
    ...
    try {
      const response = await axios.get(
        `https://ff-update-backend-6.onrender.com/api/products/search?keyword=${value}`
      );
      setSearchResults(response.data);
      ...
      <header>
        <nav className="navbar navbar-expand-lg fixed-top">
          <div className="container-fluid">
            <a className="navbar-brand" href="https://vignesh-boop.github.io/Vicky_Portfolio/">
              FLOWER FLUX UPDATE
            </a>
            <button
